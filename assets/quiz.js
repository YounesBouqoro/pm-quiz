/* ======================================================
   Projektmanagement – Kapitel 1 Quiz Engine
...
(function () {
  const RUN_SIZE = 12;
  const STATS_KEY = "pmquiz_stats_ch1";
  const RUN_KEY = "pmquiz_run_ch1";
  ...
  function mustExist(el, id) {
    if (!el) {
      console.error(`Quiz CH1: Missing element #${id}. Check chapter1.html.`);
      return false;
    }
    return true;
  }
  ...
  function showFatal(msg) {
    quizRoot.innerHTML = `
      <div class="question">
        ...
        <p class="muted">Check: In der Console <code>window.QUESTIONS_CH1?.length</code></p>
      </div>
    `;
  }
  ...
  function pickWithQuota(questions, quota) {
    // quota example: { basic:4, compare:4, transfer:3, calculation:1 }
    const byType = {};
    ...
    // If still short (not enough questions), return what we have
  ...
  function createRun() {
    ...
    if (mode === "exam" && hasTypes(questions)) {
      // Klausurmodus: Quote (anpassbar)
      const QUOTA = { basic: 4, compare: 4, transfer: 3, calculation: 1 };
  ...
  function render() {
    ...
    if (missing > 0) {
      console.warn("Quiz CH1: Run contains unknown question IDs. Recreating run.");
      clearRun();
      resetRunUI();
      run = createRun();
      if (!run) return;
    ...
    if (resultBox) {
      ...
      resultBox.innerHTML = `
        ...
        <p class="muted" style="margin-top:8px">
          Review öffnen, falsche Fragen markieren, danach direkt neuer Run. So baust du klausursichere Trefferquote.
        </p>
      `;
    }
  ...
  document.getElementById("newRun")?.addEventListener("click", () => {
    resetRun(); // ensures mode-aware creation
  });
  ...
  // If run missing, create run (mode-aware)
  if (!loadRun()) {
    createRun();
  } else {
    // If mode changed (normal <-> exam), recreate run to match mode
    const r = loadRun();
