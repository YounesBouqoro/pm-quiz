/* ======================================================
   Projektmanagement – Kapitel 3 Quiz Engine (Projektorganisation)
...
(function () {
  ...
  const STATS_KEY = "pmquiz_stats_ch3";
  const RUN_KEY   = "pmquiz_run_ch3";
  ...
  function mustExist(el, id) {
    if (!el) {
      console.error(`Quiz CH3: Missing element #${id}. Check chapter3.html.`);
      return false;
    }
    return true;
  }
  ...
  function showFatal(msg) {
    quizRoot.innerHTML = `
      <div class="question">
        ...
        <p class="muted">Check: <code>window.QUESTIONS_CH3?.length</code></p>
      </div>
    `;
  }
  ...
  function createRun() {
    ...
    if (mode === "exam" && hasTypes(questions)) {
      // Kapitel 3: Transfer heavy (Orgaformen + Rollenbegründung)
  ...
  function render() {
    ...
    if (missing > 0) {
      console.warn("Quiz CH3: Run contains unknown question IDs. Recreating run.");
      clearRun();
      resetRunUI();
      run = createRun();
      if (!run) return;
